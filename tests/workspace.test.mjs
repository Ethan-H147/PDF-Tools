...
test('opening index.html directly initializes controls and renders a PDF', { timeout: 30000 }, async () => {
  ...
  try {
    ...
    await page.locator('#pageCounter').tap();
    await page.locator('#pageCounter').tap();
    await page.locator('#pageJumpInput').fill('3');
    await page.locator('#pageJumpInput').press('Enter');
    assert.equal(Number(await page.locator('#curPage').textContent()), 3);
    await page.locator('#pageCounter').dblclick();
    await page.locator('#pageJumpInput').fill('1');
    await page.locator('#pageJumpInput').press('Escape');
    assert.equal(Number(await page.locator('#curPage').textContent()), 3);
    await page.locator('#pageCounter').dblclick();
    await page.locator('#pageJumpInput').fill('999');
    await page.locator('#pageJumpInput').press('Enter');
    assert.equal(Number(await page.locator('#curPage').textContent()), 3);
    await page.locator('#pageCounter').dblclick();
    await page.locator('#pageJumpInput').fill('1');
    await page.locator('#pageJumpInput').press('Tab');
    assert.equal(Number(await page.locator('#curPage').textContent()), 1);
